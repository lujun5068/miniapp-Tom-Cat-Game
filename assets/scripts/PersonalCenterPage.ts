...
/** 积分流水弹窗顶栏布局（标题 + 与列表间距），与 buildHistoryList 共用 */
...
const HISTORY_POPUP_HEADER_H =
  HISTORY_POPUP_TOP_PAD + HISTORY_POPUP_TITLE_H + HISTORY_POPUP_LIST_GAP;
...
export class PersonalCenterPage extends Component {
  ...
  private showHistoryPopup(): void {
    ...
    // 顶栏：标题 + 关闭（锚点顶对齐；列表区域用 HISTORY_POPUP_HEADER_H 对齐）
    const headerPadX = 24;
  ...
  private buildHistoryList(panel: Node, panelW: number, panelH: number): void {
    const viewportW = panelW - 40;
    const viewportH =
      panelH - HISTORY_POPUP_HEADER_H - HISTORY_POPUP_BOTTOM_PAD;
